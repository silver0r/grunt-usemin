...
//
FileProcessor.prototype.replaceWithRevved = function replaceWithRevved(lines, assetSearchPath, options) {
  ...
  regexps.forEach(function (rxl) {
    ...
    content = content.replace(rxl[0], function (match, src) {
      ...
      // js, css 판단 후 jsp-el 옵션이 존재 시 붙여서 치환 
      var filterOutWrapper = filterOut(file);
      var prefix = '';

      var extension = filterOutWrapper.substring(filterOutWrapper.lastIndexOf('.') + 1, filterOutWrapper.length);     
      if(extension === 'js' && options.prefix.js) {
        prefix = options.prefix.js;
      }
      else if(extension === 'css' && options.prefix.css) {
        prefix = options.prefix.css;
      }

      if(srcFile !== file) {
        filterOutWrapper = prefix + filterOutWrapper;
      }   

      var res = match.replace(src, filterOutWrapper);
      if (srcFile !== file) {
...
FileProcessor.prototype.process = function (filename, assetSearchPath, options) {
  ...
  var content = this.replaceWithRevved(this.replaceBlocks(this.file), this.file.searchPath, options);
